...
# --- integration with a real v2 model (DLinear) ---


def test_dlinear_v2_with_adamw_and_cosine_scheduler():
    try:
        from pytorch_forecasting.models.dlinear._dlinear_v2 import DLinear
    except ImportError:
        pytest.skip("DLinear v2 not available")

    metadata = {
        "context_length": 16,
        "prediction_length": 4,
        "n_features": {"continuous": 2, "categorical": 0},
        "feature_indices": {
            "continuous": [0, 1],
            "categorical": [],
            "known": [0],
