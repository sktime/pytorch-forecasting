...
def test_base_model_gradient_clipping_params():
    model = BaseModel(
        loss=MockMetric(), gradient_clip_val=0.5, gradient_clip_algorithm="norm"
    )
...
def test_configure_gradient_clipping_priority():
    model = BaseModel(
        loss=MockMetric(), gradient_clip_val=0.5, gradient_clip_algorithm="value"
    )
    ...
    model.configure_gradient_clipping(
        optimizer, gradient_clip_val=0.1, gradient_clip_algorithm="norm"
    )
...
def test_sensitive_models_defaults():
    ...
    # Samformer
    samformer = Samformer(
        loss=MockMetric(), hidden_size=16, use_revin=True, metadata=metadata
    )
