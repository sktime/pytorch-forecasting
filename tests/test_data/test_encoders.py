...
def test_PTFOrdinalEncoder_fit_transform():
    ...
    encoder = PTFOrdinalEncoder(add_unknown=True)
    data = pd.Series(["cat", "dog", "cat", "bird"])
    result = encoder.fit_transform(data)

    assert isinstance(result, torch.Tensor)
    assert result.dtype == torch.long
    assert len(result) == 4
    # Same value for same category
    assert result[0] == result[2]  # "cat" == "cat"


def test_PTFOrdinalEncoder_inverse_transform():
    """Test inverse_transform roundtrip."""
    encoder = PTFOrdinalEncoder(add_unknown=True)
    data = pd.Series(["cat", "dog", "bird"])
    encoded = encoder.fit_transform(data)
    decoded = encoder.inverse_transform(encoded)

    np.testing.assert_array_equal(decoded, data.to_numpy())


def test_PTFOrdinalEncoder_n_classes():
    """Test n_classes property."""
    encoder = PTFOrdinalEncoder(add_unknown=True)
    encoder.fit(pd.Series(["a", "b", "c"]))
    assert encoder.n_classes == 4  # 3 categories + 1 unknown


def test_PTFOrdinalEncoder_unknown_handling():
    """Test that unknown categories map to 0."""
    encoder = PTFOrdinalEncoder(add_unknown=True)
    encoder.fit(pd.Series(["a", "b"]))
    result = encoder.transform(pd.Series(["a", "c", "d"]))
    assert result[0] != 0  # "a" is known
    assert result[1] == 0  # "c" is unknown
    assert result[2] == 0  # "d" is unknown


def test_PTFOneHotEncoder_fit_transform():
    """Test fit_transform convenience method."""
    encoder = PTFOneHotEncoder(add_unknown=True)
    ...
    assert result.shape == (3, 2)  # 2 unique classes


def test_PTFOneHotEncoder_inverse_transform():
    """Test inverse_transform roundtrip."""
    encoder = PTFOneHotEncoder(add_unknown=True)
    data = pd.Series(["cat", "dog", "bird"])
    encoded = encoder.fit_transform(data)
...
def test_PTFOneHotEncoder_n_classes():
    """Test n_classes property."""
    encoder = PTFOneHotEncoder(add_unknown=True)
    encoder.fit(pd.Series(["x", "y", "z"]))
