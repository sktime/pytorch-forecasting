...
from ptf.data import TimeSeriesDataSet
...
from ptf.metrics.base_metrics import Metric
...
from ptf.utils._classproperty import classproperty
from ptf.utils._dependencies import _check_matplotlib
...
class BaseModel(InitialParameterRepresenterMixIn, LightningModule, TupleOutputMixIn):
    ...
    and follows its conventions. However, there are important additions:
        ...
        * You need to specify a ``loss`` attribute that stores the function to calculate the
          :py:class:`~ptf.metrics.MultiHorizonLoss` for backpropagation.
    ...
    def __init__(
        ...
        if not hasattr(self, "loss"):
            if isinstance(loss, tuple | list):
                self.loss = MultiLoss(
                    metrics=[
                        convert_torchmetric_to_ptf_metric(l)
                        for l in loss
                    ]
                )
            else:
                self.loss = convert_torchmetric_to_ptf_metric(loss)
    ...
    def deduce_default_output_parameters(
        ...
        Args:
            ...
            default_loss (MultiHorizonMetric, optional): default loss function.
                Defaults to :py:class:`~ptf.metrics.MAE`.
    ...
    def create_log(
        ...
        Args:
            ...
            prediction_kwargs (Dict[str, Any], optional): arguments to pass to
                :py:meth:`~ptf.models.base_model.BaseModel.to_prediction`. Defaults to {}.
            quantiles_kwargs (Dict[str, Any], optional):
                :py:meth:`~ptf.models.base_model.BaseModel.to_quantiles`. Defaults to {}.
    ...
    def forward(
        ...
        Args:
            x (Dict[str, Union[torch.Tensor, List[torch.Tensor]]]): network input (x as returned by the dataloader).
                See :py:meth:`~ptf.data.timeseries.TimeSeriesDataSet.to_dataloader` method that
        ...
        Returns:
            NamedTuple[Union[torch.Tensor, List[torch.Tensor]]]: network outputs / dictionary of tensors or list
                ...
                :py:meth:`~ptf.models.base_model.BaseModel.to_network_output` method.
                ...
                * ``prediction`` (batch_size x n_decoder_time_steps x n_outputs or list thereof with each
                ...
                :py:meth:`~ptf.models.base_model.BaseModel.transform_output`
        ...
        **kwargs,
        ...
        Args:
            ...
            target: Defines which values are overwritten for making a prediction.
                Same as in :py:meth:`~ptf.data.timeseries.TimeSeriesDataSet.set_overwrite_values`.
                Defaults to "decoder".
