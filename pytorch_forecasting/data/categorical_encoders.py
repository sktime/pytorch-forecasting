...
class CategoricalEncoderMixin:
    ...
    def _to_numpy(self, y: Union[pd.Series, np.ndarray, torch.Tensor]) -> np.ndarray:
...
class PTFOrdinalEncoder(BaseEstimator, TransformerMixin, CategoricalEncoderMixin):
    ...
    Maps unknown categories during transformation to an unseen class representation (0 or max_class).
    """
    ...
    def fit(self, y: Union[pd.Series, np.ndarray, torch.Tensor]) -> "PTFOrdinalEncoder":
    ...
    def transform(self, y: Union[pd.Series, np.ndarray, torch.Tensor]) -> torch.Tensor:
        y_np = self._to_numpy(y)
...
class PTFOneHotEncoder(BaseEstimator, TransformerMixin, CategoricalEncoderMixin):
    ...
    def fit(self, y: Union[pd.Series, np.ndarray, torch.Tensor]) -> "PTFOneHotEncoder":
        y_np = self._to_numpy(y)
