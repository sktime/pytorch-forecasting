...
class Softs(TslibBaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.n_quantiles = len(loss.quantiles) if hasattr(loss, "quantiles") and loss.quantiles is not None else 1
    ...
    def _init_network(self, d_model, d_core, d_ff, n_layers, dropout):
        ...
        self.encoder = nn.ModuleList([
            SoftsEncoderLayer(d_model=d_model, d_core=d_core, d_ff=d_ff, dropout=dropout)
            for _ in range(n_layers)
        ])

        # 4. Final Projection
        self.projection = nn.Linear(self.context_length * d_model, self.prediction_length * self.n_quantiles)
