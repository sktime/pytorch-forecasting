...
class mLSTMNetwork(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size, dropout=0.0, use_layer_norm=True,
                 use_residual=True, device=None):
        super(mLSTMNetwork, self).__init__()
        self.device = device or torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        self.mlstm_layer = mLSTMLayer(input_size, hidden_size, num_layers, dropout, use_layer_norm, use_residual,
                                      self.device)
