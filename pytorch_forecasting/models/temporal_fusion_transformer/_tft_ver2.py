...
class TemporalFusionTransformer(BaseModelWithCovariates):
    def __init__(
        ...
        lstm_layers: int = 1,
        dropout: float = 0.1,
        output_size: Union[int, List[int]] = None,
        ...
        * non-linear variable processing in variable selection network can be shared among decoder and encoder
        ...
        static_categoricals = [
            f"static_cat_{i}"
            for i in range(metadata.get("static_categorical_features", 0))
        ]
        static_reals = [
            f"static_real_{i}"
            for i in range(metadata.get("static_continuous_features", 0))
        ]
        time_varying_categoricals_encoder = [
            f"encoder_cat_{i}" for i in range(metadata["encoder_cat"])
        ]
        time_varying_reals_encoder = [
            f"encoder_real_{i}" for i in range(metadata["encoder_cont"])
        ]
        time_varying_categoricals_decoder = [
            f"decoder_cat_{i}" for i in range(metadata["decoder_cat"])
        ]
        time_varying_reals_decoder = [
            f"decoder_real_{i}" for i in range(metadata["decoder_cont"])
        ]
        x_categoricals = (
            static_categoricals
            + time_varying_categoricals_encoder
        ...
        x_reals = static_reals + time_varying_reals_encoder + time_varying_reals_decoder
        ...
        self.hparams.max_encoder_length = max_encoder_length
        self.hparams.output_size = output_size
        self.hparams.static_categoricals = static_categoricals
        self.hparams.static_reals = static_reals
        ...
        self.hparams.time_varying_reals_encoder = time_varying_reals_encoder
        self.hparams.time_varying_reals_decoder = time_varying_reals_decoder
        self.hparams.x_categoricals = x_categoricals
        self.hparams.x_reals = x_reals
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if encoder_cont.shape[-1] != decoder_cont.shape[-1]:
            max_features = max(encoder_cont.shape[-1], decoder_cont.shape[-1])

            if encoder_cont.shape[-1] < max_features:
                encoder_padding = torch.zeros(
                    encoder_cont.shape[0],
                    encoder_cont.shape[1],
                    max_features - encoder_cont.shape[-1],
                ).to(encoder_cont.device)
                encoder_cont = torch.cat([encoder_cont, encoder_padding], dim=-1)

            if decoder_cont.shape[-1] < max_features:
                decoder_padding = torch.zeros(
                    decoder_cont.shape[0],
                    decoder_cont.shape[1],
                    max_features - decoder_cont.shape[-1],
                ).to(decoder_cont.device)
                decoder_cont = torch.cat([decoder_cont, decoder_padding], dim=-1)
        x_cont = torch.cat(
            [encoder_cont, decoder_cont], dim=1
        )  # concatenate in time dimension
        ##########
        ...
        ############different from _tft###########################
        available_reals = [name for name in self.hparams.x_reals if name in self.reals]
        max_features = x_cont.shape[-1]

        input_vectors.update(
            {
                name: x_cont[..., idx].unsqueeze(-1)
                for idx, name in enumerate(available_reals)
                if idx < max_features
            }
        )

        all_expected_vars = set(self.encoder_variables + self.decoder_variables)
        real_vars_in_input = set(input_vectors.keys())
        missing_vars = all_expected_vars - real_vars_in_input

        for var_name in missing_vars:
            if var_name.startswith(("encoder_real_", "decoder_real_")):
                # Create zero tensor with same shape as other real variables
                zero_tensor = torch.zeros_like(x_cont[..., 0].unsqueeze(-1))
                input_vectors[var_name] = zero_tensor
        ############
