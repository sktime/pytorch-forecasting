...
class DeepAR(BaseModel):
    ...
    output_transformer : Any, optional
        Transformer to use for inverse transforming predictions.
    """
    ...
    def output_transformer(self):
        """Getter for the transformer, defaults to hparams or None."""
        return getattr(
            self, "_output_transformer", self.hparams.get("output_transformer", None)
        )

    @output_transformer.setter
    def output_transformer(self, value):
        """Setter that keeps the model and its saved hyperparameters in sync."""
        self._output_transformer = value
        self.hparams.output_transformer = value
